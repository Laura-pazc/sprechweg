...
4. **Register** — practical / bureaucratic / friendly / formal (see table below).
   This *determines* the target language's formal/informal address form (if it
...
prompts, recall-check intro lines) is the *app's* coaching voice speaking to the
...
2. **Mission-specific unblock** — the one real thing that gets *this* learner
   ready for *this* errand (a real hotline number, a real portal type, a branch
...
One intro line noting this happens *after* the real errand and isn't a full
...
*next* CEFR level brings to this same scenario — not just "more vocabulary."
